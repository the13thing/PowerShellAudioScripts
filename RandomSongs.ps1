...
function Rename-File {
    ...
    switch ($RenameScheme) {
        ...
        "number-title"        { return "$number $title$($File.Extension)" }

        "number-artist-title" { return "$number $artist - $title$($File.Extension)" }

        "artist-title"        { return "$artist - $title$($File.Extension)" }
    ...
    foreach ($f in $Files) {
        ...
        if ($ConvertToMp3 -and $f.Extension.ToLower() -eq ".flac") {
            ...
            if (-not $DryRun) {
                ...
                ffmpeg -y -loglevel error -i "`"$($f.FullName)`"" -codec:a libmp3lame -qscale:a 2 `

                    -map_metadata 0 -id3v2_version 3 -map 0:a -map 0:v? -disposition:v attached_pic `

                    -c:v mjpeg -vf "scale=300:300:force_original_aspect_ratio=decrease" -pix_fmt yuvj420p "`"$destFullPath`"" 2>$null
